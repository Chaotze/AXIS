...
//   相对路径，保证 unitest 中语义一致
// - 本模块是唯一持有全局状态（Spinlock<Option<TaskState>>）
//   的地方；锁序：TASK 为叶（不向内持有其他全局锁）
//
// 本轮交付边界（在 arch.md 中修订说明）：
// - 数据结构与算法完整：进程树、生命周期、信号、rlimit、
//   命名空间、cgroup、CFS 就绪队列、抢占判定、亲和性、
//   负载均衡——全部经宿主单元测试验证
// - 真实上下文切换（多任务并发运行）依赖 arch 层接线
//   （TrapFrame + switch.asm），是下一轮任务；tick 钩子
//   预留接口 tick_hook()
// - 用户态（fork/exec 真实语义、Hello World 用户程序）
//   依赖系统调用层（阶段 8）与兼容层（阶段 9）
